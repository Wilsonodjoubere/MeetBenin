    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-white py-3 border-bottom">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-light text-dark fs-6">{{ $typesMedia->count() }} type(s)</span>
            </div>
        </div>
                ...
                <table class="table table-hover align-middle mb-0" id="typesMediaTable">
                    ...
                    <tbody>
                        ...
                        <tr id="type-{{ $type->id }}">
                            ...
                            <td class="editable" onclick="editCell(this, 'nom', {{ $type->id }})">
                                <div class="d-flex align-items-center">
                                    <i class="bi bi-file-earmark-medical me-2 text-primary"></i>
                                    <span class="type-nom fw-medium">{{ $type->nom }}</span>
                                </div>
                            </td>
                            
                            <td class="editable" onclick="editCell(this, 'code', {{ $type->id }})">
                                <span class="badge bg-primary type-code px-2 py-1">
                                    {{ $type->code }}
                                </span>
                            </td>
                            
                            <td class="editable" onclick="editCell(this, 'description', {{ $type->id }})">
                            ...
                            <td class="text-center">
                                <span class="badge bg-{{ $type->statut ? 'success' : 'danger' }} type-statut">
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm" role="group">
                                    ...
                                    <!-- Bouton Voir -->
                                    <a href="{{ route('types-media.show', $type->id) }}" 
                                    ...
                                    <!-- Bouton Supprimer -->
                                    <form action="{{ route('types-media.destroy', $type->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
        ...
        <!-- Pied de table -->
        @if($typesMedia->isNotEmpty())
        <div class="card-footer bg-white py-3">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <p class="mb-0 text-muted small">
                        Affichage de <strong>{{ $typesMedia->count() }}</strong> type(s)
                    </p>
                </div>
